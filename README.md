...
- 2. App.jsx - __first part__ - 
  ...
  - for now, in it just add render methode that will render what we write and return inside that will call that methode so that it actually renders what we wrote:
  ...
  - First, container that will hold it all, <div> with className of App
    ...
    - we need ***header*** that will hold things that can navigate us trough the site
    - ***main***, that will have our main content
    - ***footer***, that has in this case only copyright clame, but can hold, navigation links, email input 
    for something like newletter and more
    <!-- those 3 dont need classes in this case because there is just one for every page, in larger project, sometimes people use classes to make different mains for different pages, but in most cases main is just container that you fill and does not need a class(or ID) because it is unicqe, with classes you mostly declaser some attributes that that class will have and add deffined class to existing element to change it like in bootstrap or talewind. There classes are used like tools, and you can add one on top of the other to edit your code without ever needing to go to css file, because everything is already predeclared there. -->
      ...
      - in ***header***, for this case, we'll just put _logo-section_
        - in _logo-section_, needs to be an img element, that will have sorce of our logo file and ClassName of _logo_, height and width of **40**
      - and empty ***nav*** section, that in this case will not be needed yet but we added it for later upgades
      - in ***main***, we'll need _chat-rooms-section_ that will for now be empty, for future updates
      - and _chat-section_ 
       - that will have _messages-div_
       - and _input-div_
      - in ***footer***, there will be just a paragraph, within which will be our copywrite clame
...
- 3. input.jsx - first part - 
  ...
  - inside it:
    ...
    - next we need couple of methodes:
      - **_onChange()_**
      ...
      - **_onSubmit()_**
        - and in submit, 
          ...
          **_ this.props.onSendMessage(this.state.text); _** - to get it to methode in app that will catch that message
        
      <!-- that act as out eventListeners -->
      - and a render() methode just like in ChatApp, with return() to call what we wrote inside
        - container that holds everything <div className="Input">
          - form,  <form onSubmit={e => this.onSubmit(e)}>, with onSubmit attribute that is a function that calls onSubmit methode
            - input, <input
            **_onChange={e => this.onChange(e)}_** - calls onChange methode
  ...
  - go to MessageBubbles file and create class MessageBubbles:
    ...
    - inside it: 
      - to scroll to the end of the messages every time new message comes:
        **_scrollToBottom = () => { _** - function for scrolling automaticlly to bottom on every new message
        ...
        **_  this.scrollToBottom(); _**
        **_} _**
      - render methode, and return, HTML structure for messages: 
        **_render() { _** - opens render methode
        **_  const {messages} = this.props; _** - declares messages, that will later be used as prop to pass data trough
        **_  return ( _** - opens return  that will return HTML inside to render methode and it will render that on the page
      ...
      - generating messages for messages array:
          **_renderMessage(message) { _** - methode that will take text from input and send them to group, and render them properly
          ...
          **_    <div className={className}> _** - div that constains eather one or the other class combination
          **_    <span _** - icon of the user with class avatar, and that icon rendomized color specific to that member 
...
- 4. ChatApp.jsx - second part -
  ...
  - arrays for username:
    ...
    **_  ]; _**
    **_}; _**
  - random number generators, that choose colour and username:
  ...
  - state for ChatApp:
    ...
    **_state = { _**
      <!-- // 2 parametars, messages, by member, and member who sends them -->
      **_messages: [], _**
      **_member: { _**
        <!-- // member is also an object, and has to parametars, username of that member, and color, for icon -->
        <!-- // both username and color are randomised from functions above that are called in here -->
 ...
 ----------- CSS --------------
 I would be as bold and say that CSS is readable enaugh so that it doesn't need it own description, but in a nutshell:
  - this app was from CSS point structured mostly as a grid when I needed large peaces to fold into place and for smaller elements that contain just text or similar I used flex
  - general differences between this two is that with grid you can create shapes and divisions that are not eaquel to each other but still follow some general roles, and with flex thing in a way just fall into place by them selves
  ...
  - cons are that to do it well you need a bit larger learning curve and for some specific usecases you need to write a lot more css for same effect as some other options 

  - pros for flex is that is relativly easy to learn, especialy if you work in chrome where you can click on flex icon in inspector and click trough options that flex ofers, in that way you can pick up on different ways flex works very quickly, and for smaller layouts, where you just need everything to be tighty, I would say it's, most of the time, perfect
  - cons, for bigger layouts it uses, eather to many divisions to get the same effect as grid or to much space if you use it as "intended"

  - there, of course are other smaller things like position that can be very usefull but it's a bit old now, and i didn't really got to mesurements, but in short, there are to ways to declare some lenght, with fixed increments, like px or cm, mm, or inch-es, and there are modular ones, like em, rem, %, vh, vw and so on, there are some benefites to both, but most of the time modular ones are better mostly because you don't know on whos device will it be and how large is screen on it.
